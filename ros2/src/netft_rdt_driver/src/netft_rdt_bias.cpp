...
NetFTRDTDriverBias::NetFTRDTDriverBias(double rot,
                                       const tf2::Vector3& scale_F, double alpha, std::size_t num_points):
  Node("netft_rdt_driver_bias"),
  alpha(alpha),
  scale_F(scale_F),
  num_points(num_points)
{
  ...
  rclcpp::Service<netft_rdt_driver::srv::StringCmd>::SharedPtr service = this->create_service<netft_rdt_driver::srv::StringCmd>("bias_cmd",   std::bind(&NetFTRDTDriverBias::service_callback, this, 
                                                                                                                                                          std::placeholders::_1, std::placeholders::_2)); 

  pub_bias_status = this->create_publisher<std_msgs::msg::Bool>("bias_status",1);
  pub_bias = this->create_publisher<geometry_msgs::msg::Wrench>("bias", 1);
...
bool NetFTRDTDriverBias::service_callback(const std::shared_ptr<netft_rdt_driver::srv::StringCmd::Request> request,
